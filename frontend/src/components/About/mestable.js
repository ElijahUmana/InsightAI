...
function About() {
    ...
    useEffect(() => {
        if (audioChunkCounter > 0) {
            setShowVideo(true); // Start the video when there's an audio chunk to play
        }
    ...
    useEffect(() => {
        const fetchProcessedImage = async () => {
            try {
                const response = await axios.get('http://localhost:5001/get-processed-image');
...
useEffect(() => {
    ...
    const handleKeyDown = async (e) => {
        if (e.code === 'Space' && !isRecording && !isStopping) {
            setCurrentCount(3); // Start the countdown immediately upon pressing space

            // A small delay to initiate the recording slightly after the countdown starts
            setTimeout(async () => {
                await run();
                ...
                }, 700); // 700ms divided by 3 for the 3-second countdown

            }, 50); // A 50ms delay for the recording to start after initiating the countdown
        }
    };
    ...
    const playBufferedAudio = () => {
        if (audioBufferQueue.length === 0) {
            // Since the queue is empty, check if the audio context has finished playing
    ...
    const startResponseSocket = () => {
        const responseSocket = new WebSocket('http://localhost:5001/ws');
            ...
            assemblySocketRef.current.onopen = () => {
                navigator.mediaDevices.getUserMedia({ audio: true })
                    .then((stream) => {
                        ...
                        gainNode.gain.value = 3;  // Increase volume
                ...
                <div className="chat-box">
                    <div className="flex-container">
                        ...
                        <div className="media-container">
                            {displayText && <h3 className="chat-text" style={{ color: "white", fontWeight: "lighter" }}>Hi there! Ask me any question about your image</h3>}
                            {displayVideo && (
